...
def performance_analyzer(parsed_frames):
    ...
    while True:
        ...
        if choice not in ['y','yes','yea', 'sure', 'rip steve jobs', 'i hate tim cook', 'i love Apple', 'i love the first 5g iphone']:
...
def plot_rssi(parsed_frames):
    ...
    if 'rssi' in df.columns:
        ...
        x = list(range(len(df['rssi'])))
        values = df['rssi'].values

        plt.figure(figsize=(20, 15))
        plt.plot(x, values, color='royalblue', marker='o', markersize=3, linestyle='-', linewidth=1)

        plt.title("Time Series Plot of RSSI", fontsize=16)
        plt.xlabel("Frame Index", fontsize=14)
        plt.ylabel("RSSI (dBm)", fontsize=14)
        plt.grid(True, linestyle='--', alpha=0.3)
        plt.xticks(ticks=x[::100])
        plt.tight_layout()
        plt.legend(["RSSI"], loc='lower right', fontsize=10, frameon=True)
        plt.show()
...
def main():
    filepath = "../captures/HowIWiFi_PCAP.pcap"
